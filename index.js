...
app.post("/stusearch", async (req, res) => {
    const { stusearch } = req.body;
    console.log(req.body)
    const results = await FormData.find({
        $or: [
            { name: stusearch },
            { branch: stusearch },
            { companyname: stusearch },
            { collegename: stusearch }
        ]
    });
    console.log(results)
    res.render("studentdisplay", { data: results });
});
